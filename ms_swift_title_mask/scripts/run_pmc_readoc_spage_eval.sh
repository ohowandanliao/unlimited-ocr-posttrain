#!/usr/bin/env bash
# run_pmc_readoc_spage_eval.sh — pmc-readoc-spage-16k 训完一键评测（129 全管线）+ 自动生成支线评测文档
#
# 用法:
#   bash run_pmc_readoc_spage_eval.sh                 # 等训练结束自动开评（GPU1）
#   bash run_pmc_readoc_spage_eval.sh --skip-wait     # 训练已结束，直接开评
#   RUN_DIR=/path/to/run bash run_pmc_readoc_spage_eval.sh --skip-wait   # 指定 checkpoint 所在 run 目录
#
# 管线: 等训→起服务推理129→剥壳(pred_clean)+重建标题(pred_titles,均过--check)→逐篇sweep(swp3_,缺口容错)
#       →聚合→合成→AgentBuilder→title S5(87篇)+Overall splice+退化四板斧→写评测目录报告
...
RUN_NAME=pmc-readoc-spage-16k
EVAL=$UOCR_ROOT/evaluation/${RUN_NAME}_all129
PREFIX=swp3_
PORTS=(18191 18192 18193 18194)
...
(( GPU_USED > 2000 )) && { echo "[P0] FATAL: GPU1 busy (${GPU_USED}MiB)"; exit 1; }
fi
...
ev = Path("/home/jovyan/hyx/uocr-ms-swift-title-mask/evaluation/pmc-readoc-spage-16k_all129")
out = ev / "pred" / "pmc-readoc-spage-16k"; out.mkdir(parents=True, exist_ok=True)
n = 0
...
PYEOF
PC_DIR="$EVAL/pred/pmc-readoc-spage-16k"
CLEAN_DIR="$EVAL/pred_clean/pmc-readoc-spage-16k"
# 2026-09-18 人工复核记录: 3 篇为畸形 grounding 脚手架(未闭合<|det|>残留, 正文完好),
# conservative 拒绝落盘; 按协议经复核后对确认文件用 --aggressive 清理(正文永不删)并在此记录。
AGGRESSIVE_FILES=${AGGRESSIVE_FILES:-"1-3单元格内换行2.md 4-2段落中带「图x-x」.md 5-1双栏表格1.md"}
...
python3 "$STRIP" --check --in-dir "$EVAL/pred_clean/pmc-readoc-spage-16k" || exit 1
python3 "$STRIP" --rebuild-titles --in-dir "$EVAL/pred/pmc-readoc-spage-16k" --out-dir "$EVAL/pred_titles/pmc-readoc-spage-16k" || exit 1
python3 "$STRIP" --check --in-dir "$EVAL/pred_titles/pmc-readoc-spage-16k" || exit 1
echo "[P2] pred_clean + pred_titles ready and check-passed"

# ---------- P3 逐篇 sweep（swp3_，4 并发，600s 超时，缺口容错）----------
...
WORK=/tmp/rswp3; rm -rf "$WORK"; mkdir -p "$WORK"
PC=$EVAL/pred_clean/pmc-readoc-spage-16k
...
run_one() {
  ...
  ( cd "$OMNI" && timeout 600 "$VENV" run_md2md.py --gt_dir "$gt" --pred_dir "$pd" \
      --save_name "swp3_$D" > "$WORK/log_$D.txt" 2>&1 )
...
EV = U / "evaluation/pmc-readoc-spage-16k_all129"
...
def pooled(suffix, agg):
    a, k = [], []
    for p in OMNI.glob(f"swp3_*{suffix}"):
...
# 验证门
ts_p = Path("/home/jovyan/hyx/uocr-ms-swift-title-mask/output/pmc-readoc-spage-16k")
sh = sorted(ts_p.glob("v0-*/v0-*/checkpoint-*/trainer_state.json"),
            key=lambda p: int(p.parts[-3].split("-")[-1]))[-1]
...
res = Path("/tmp/rswp3/results.txt")
if res.is_file():
...
doc.append("- 2026-09-18 复核记录：3 篇（1-3单元格内换行2 / 4-2段落中带「图x-x」 / 5-1双栏表格1）模型输出为畸形 grounding 脚手架（未闭合 <|det|> 残留，conservative 拒绝落盘），经复核正文完好后按协议用 --aggressive 清理脚手架，正文未删。")
