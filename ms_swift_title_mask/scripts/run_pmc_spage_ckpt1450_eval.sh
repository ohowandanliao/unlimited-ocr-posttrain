#!/usr/bin/env bash
# run_pmc_spage_ckpt1450_eval.sh — pmc-readoc-spage-16k ckpt-1450 诊断评测（129 全管线）
#
# 目的: 步数消融——同 mix 配方,1450 vs 1623 步为纯步数增量;对照 pmc16k@1160(异配方同步数邻域)。
# 由 run_pmc_readoc_spage_eval.sh 复制修改: RUN_NAME/PREFIX(swp4_)/AGGRESSIVE_FILES。
# GPU1 门放宽(外部常驻 ~36GB 空闲占用,余 45GB 够推理)/退化检测与 train_loss 门取 ckpt-1450 自己的
# trainer_state(经 /tmp/ckpt1450_run 符号链接传入 RUN_DIR)。
#
# 用法:
#   RUN_DIR=/tmp/ckpt1450_run bash run_pmc_spage_ckpt1450_eval.sh --skip-wait
#
# 管线: 起服务推理129→剥壳(pred_clean)+重建标题(pred_titles,均过--check)→逐篇sweep(swp4_,缺口容错)
...
RUN_NAME=pmc-readoc-spage-ckpt1450
...
ev = Path("/home/jovyan/hyx/uocr-ms-swift-title-mask/evaluation/pmc-readoc-spage-ckpt1450_all129")
out = ev / "pred" / "pmc-readoc-spage-ckpt1450"; out.mkdir(parents=True, exist_ok=True)
n = 0
...
# 2026-09-21 人工复核记录(ckpt-1450 轮): 3 篇各恰含 1 个未闭合 <|det|> 块(open=close+1),
# 其余块全部闭合、正文完好——与 2026-09-18 mix@1623 轮同名单同性质;按坑14 经复核后
# 对确认文件用 --aggressive 清理脚手架(正文永不删)。
...
# ---------- P3 逐篇 sweep（swp4_，4 并发，600s 超时，缺口容错）----------
...
WORK=/tmp/rswp4; rm -rf "$WORK"; mkdir -p "$WORK"
...
run_one() {
  ...
  ( cd "$OMNI" && timeout 600 "$VENV" run_md2md.py --gt_dir "$gt" --pred_dir "$pd" \
      --save_name "swp4_$D" > "$WORK/log_$D.txt" 2>&1 )
...
EV = U / "evaluation/pmc-readoc-spage-ckpt1450_all129"
...
def pooled(suffix, agg):
    a, k = [], []
    for p in OMNI.glob(f"swp4_*{suffix}"):
...
# 验证门(train_loss 取 ckpt-1450 自己的 trainer_state,即截至 1450 步的历史)
ts_p = Path("/tmp/ckpt1450_run")
...
res = Path("/tmp/rswp4/results.txt")
if res.is_file():
...
doc.append("- 本轮 AGGRESSIVE_FILES 为三篇人工复核白名单；仅清理 grounding 脚手架，正文不删。")
